...
function GoogleLogin() {
  ...
  return (
    <button
      ...
      className="mt-2 text-black hover:bg-[#d7e4eb] bg-[#ddecf5] border px-5 p-2 rounded-xl flex gap-3 items-center justify-center"
      disabled={isSigningIn} // Disable button when signing in
    >
      {isSigningIn ? (
        <div className="flex items-center w-full  justify-center h-[25px]">
          <ClipLoader color="#000" loading={true} size={17} />
        </div>
      ) : (
        <>
          <h2 className="font-semibold text-[1rem] max-[500px]:text-[0.9rem]">
            Sign in with google
          </h2>
          <svg
